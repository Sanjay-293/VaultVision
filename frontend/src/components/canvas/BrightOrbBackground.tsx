...
function SmallFloatingGeometries() {
  ...
  const geometries = useMemo(() => {
    ...
    return Array.from({ length: count }).map((_, i) => ({
      position: [
        (seededRandom() - 0.5) * positionSpread,
        (seededRandom() - 0.5) * positionSpread,
        (seededRandom() - 0.5) * positionSpread - 5
      ] as [number, number, number],
      type: i % 3
    }));
  }, [count, positionSpread]);
  ...
  return (
    <group ref={group}>
      {geometries.map((geo, i) => (
        ...
        >
          {geo.type === 0 ? <boxGeometry args={[0.5, 0.5, 0.5]} /> : geo.type === 1 ? <octahedronGeometry args={[0.4]} /> : <torusGeometry args={[0.3, 0.1, 16, 32]} />}
          <MeshTransmissionMaterial 
            thickness={0.2}
            roughness={0}
            transmission={1}
