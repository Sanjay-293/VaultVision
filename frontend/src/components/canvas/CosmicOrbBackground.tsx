...
function MediaCard({ angle, radius, delay, imageUrl, title, status }: { angle: number, radius: number, delay: number, imageUrl: string, title: string, status: string }) {
  ...
  useFrame((state) => {
    ...
    if (pulseRef.current) {
      ...
      (pulseRef.current.material as THREE.MeshBasicMaterial).opacity = 0.2 + Math.sin(time * 4) * 0.1;
    }
  });
...
function ShootingStar({ delay, imageUrl }: { delay: number, imageUrl: string }) {
  ...
  const { startPos, velocity } = useMemo(() => {
    ...
    return {
      startPos: [
        (random() - 0.5) * 50, // wide X spawn
        (random() - 0.5) * 40, // wide Y spawn
        (random() - 0.5) * 20 - 15 // depth
      ],
      velocity: [
        (random() - 0.5) * 100, // random X direction
        (random() - 0.5) * 80,  // random Y direction
        (random() - 0.5) * 10   // slight Z drift
      ]
    };
  }, [delay]);
  ...
  useFrame((state) => {
    ...
    if (ref.current) {
      ...
      if (trail && (trail.material as THREE.MeshBasicMaterial)) {
        (trail.material as THREE.MeshBasicMaterial).opacity = progress > 0.8 ? (1 - progress) * 5 : 0.6;
      }
      if (newsCard && (newsCard.material as THREE.MeshBasicMaterial)) {
        (newsCard.material as THREE.MeshBasicMaterial).opacity = progress < 0.2 ? (progress - 0.1) * 10 : progress > 0.7 ? (1 - progress) * 3 : 0.8;
      }
    }
  });
